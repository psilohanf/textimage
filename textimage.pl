...
$num_args = $#ARGV + 1;
die "usage: textimage \"quote\" \"author\"\n" unless $num_args == 2;
...
system("convert -background '#$bg[$theme]' -fill '#$f1[$theme]' -size 320x430 -font Times-Italic caption:'\"$quote\"' -trim -font Times-Roman -gravity SouthEast -size 320x -pointsize 30 -fill '#$f2[$theme]' caption:'($author)' -trim -bordercolor '#$bg[$theme]' -border 30 -append quote.gif");
